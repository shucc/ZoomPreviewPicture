package com.previewlibrary.widget;
...
public class BezierBannerView extends View implements ViewPager.OnPageChangeListener {
    ...
    //背景圆初始半径
    private float mNormalRadius = 20;
    //背景圆变化半径 背景圆都用这个
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        mSelectedColor = typedArray.getColor(R.styleable.BezierBannerView_bezier_banner_selectedColor, 0xFFFFFFFF);
        mUnSelectedColor = typedArray.getColor(R.styleable.BezierBannerView_bezier_banner_unSelectedColor, 0xFFAAAAAA);
        mRadius = typedArray.getDimension(R.styleable.BezierBannerView_bezier_banner_selectedRadius, mRadius);
        mNormalRadius = typedArray.getDimension(R.styleable.BezierBannerView_bezier_banner_unSelectedRadius, mNormalRadius);
        distance = typedArray.getDimension(R.styleable.BezierBannerView_bezier_banner_spacing, distance);
        typedArray.recycle();
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int width = (int) (mNormalRadius * 2 * count + (mRadius - mNormalRadius) * 2 + (count - 1) * distance + getPaddingLeft() + getPaddingRight());
        ...
        for (int i = 0; i < count; i++) {
            if (mDirection == DIRECTION_RIGHT) {
                if (i == mSelectedIndex || i == mSelectedIndex + 1) {
                    ...
                    canvas.drawCircle(getCenterPointAt(i), mRadius, mNormalRadius, mCirclePaint2);
                }
    ...
    private void moveToNext() {
        ...
        //----------------------背景圆反方向移动--------------------------------
        //起始圆圆心
        mbgNextPointX = getValue(getCenterPointAt(mSelectedIndex + 1), getCenterPointAt(mSelectedIndex) + mNormalRadius, MOVE_STEP_TWO);
        mbgNextPointY = mRadius;
        //起始圆半径
        mChangeBgRadius = getValue(mNormalRadius, 0, mRadiusProgress);
        ...
        //辅助圆圆心
        mSupportNextCenterX = getValue(getCenterPointAt(mSelectedIndex + 1) - mNormalRadius, getCenterPointAt(mSelectedIndex), MOVE_STEP_ONE);
        mSupportNextCenterY = mRadius;
        //辅助圆半径
        mSupportNextChangeRadius = getValue(0, mNormalRadius, mRadiusProgress);

        //终点与辅助圆圆心间的角度
        double mSupport_Next_radian = Math.toRadians(getValue(0, 45, MOVE_STEP_TWO));
        float mSupport_Next_radianX = (float) (Math.sin(mSupport_Next_radian) * mSupportNextChangeRadius);
        float mSupport_Next_radianY = (float) (Math.cos(mSupport_Next_radian) * mSupportNextChangeRadius);
        ...
        //终点
        float endPoint_support_nextX = mSupportNextCenterX + mSupport_Next_radianX;
        float endPoint_support_nextY = mSupportNextCenterY - mSupport_Next_radianY;

        //控制点
        float controlPointX_Next = getValueForAll(getCenterPointAt(mSelectedIndex + 1) - mNormalRadius, getCenterPointAt(mSelectedIndex) + mNormalRadius);
    ...
    private float getCenterPointAt(int index) {
        ...
        return index * (distance + 2 * mNormalRadius) + mNormalRadius + (mRadius - mNormalRadius);
    }
